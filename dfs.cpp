...
void pushFringeElements(map* elements){
    if(elements->left != NULL)
        adjacent.push(elements->left);
    if(elements->right != NULL)
        edge.push(elements->right);
}
...
int main(){
    ...
    adjacent.push(&elements);

    while(!adjacent.empty()){
        currentElement = adjacent.top();
        adjacent.pop();
        ...
        if(adjacent.empty() && !edge.empty()){
            adjacent.push(edge.top());
            edge.pop();
        }
    }
